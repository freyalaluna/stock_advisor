...
joined = (allStocks.join(allHeadlines, on=[allStocks.ticker == allHeadlines.ticker, allStocks.start_date == allHeadlines.date], how="left")
                   .drop(allHeadlines.ticker)
                   .drop("date"))

#Squash all duplicate rows, aggregating the headline scores that occurred on the same day
squashed = (joined.groupBy(["ticker", "start_date"]).agg(sf.first("end_date").alias("end_date"),
...
timeframe = Window.partitionBy("ticker").orderBy("start_date").rowsBetween(-5,Window.currentRow)
...
stocksWithHeadlines = (stocksWithHeadlines.withColumns({"last_five_sentiments": sf.slice(stocksWithHeadlines.last_five_sentiments, 1, 10),
                                                       "last_five_score": sf.slice(stocksWithHeadlines.last_five_score, 1, 10)})
